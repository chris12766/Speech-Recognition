...
def decode_audio(wav_file, desired_samples=-1):
    ...
    with tf.name_scope('decode_audio'):
        ...
        audio, sampling_rate = contrib_audio.decode_wav(
...
class AudioSpectro():
    ...
    """
    def __init__(self, audio_length, sampling_rate, frame_size, frame_stride,
                             num_mel_bins):
        ...
        self._spectro_data = audio_to_spectrogram(
                self._audio, sampling_rate, frame_size, frame_stride,
...
def nn_optimizer(loss, l2_scale, init_lr, dec_rate_lr, lr_drate, dbg=False):
    ...
    with tf.name_scope('optimizer'):
        max_grad_norm = 10.0
        learning_rate = tf.train.exponential_decay(init_lr, global_step, dec_rate_lr,
...
class AudioNN():
    ...
    """
    def __init__(self, FLAGS, num_classes, max_label_length, dec_rate_lr,
                             is_training, log_dir):
        sampling_rate = FLAGS.sampling_rate
        audio_length = int(sampling_rate * FLAGS.target_duration_ms / 1000)
        frame_size = int(sampling_rate * FLAGS.frame_size_ms / 1000)
        frame_stride = int(sampling_rate * FLAGS.frame_stride_ms / 1000)
        ...
        nn_input = audio_to_spectrogram(
                self._audio, sampling_rate, frame_size, frame_stride, num_mel_bins)
        ...
        self._train_step, self._global_step = nn_optimizer(
                self._loss, l2_scale, init_lr, dec_rate_lr, lr_drate)
